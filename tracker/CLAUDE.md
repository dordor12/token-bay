...
- `internal/<module>/` — server, api, registry, broker, session, ledger, federation, reputation, stunturn, admin, config, metrics
...
- `internal/broker`, `internal/session`, `internal/federation`, `internal/admission`, and `internal/reputation` are the most concurrent modules. Run `go test -race` by default; flakes there are always real bugs.
