...
// --- Per-handler narrow interfaces and stub installers. -----------------
//
// Each interface declares the slice of a subsystem one handler needs.
// The matching install* function returns a notImpl closure when Deps.X
// is nil; Phase 3 replaces each install* body with real handler logic
// in its own file (and moves the interface there too).
...
type advertiseRegistry interface {
	advertiseRegistryMarker() // empty marker; Phase 3 introduces the real method.
}
...
type turnAllocator interface {
	turnAllocatorMarker() // empty marker; Phase 3 fills with stunturn.Allocate.
}
